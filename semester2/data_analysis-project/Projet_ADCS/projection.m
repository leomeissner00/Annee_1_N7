...
%Initialisation de X_reconstruit
X_reconstruit = repmat(individu_moyen,16,1); %individu_moyen;

for q = 1:n-1
    Cq = C(:,1:q);	% q premières composantes principales
    Wq = W(:,1:q);	% q premières eigenfaces

    X_reconstruit = X_reconstruit + Cq*Wq';
    figure(1);
    ...
    RMSE = sqrt(mean((X - X_reconstruit).^2, 'all'));
