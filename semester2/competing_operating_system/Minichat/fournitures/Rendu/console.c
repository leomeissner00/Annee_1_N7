...
int main (int argc, char *argv[]) {
    ...
    strcpy(tubeS2C, strcat(pseudo,"_S2C"));
    mkfifo(tubeS2C,S_IRUSR|S_IWUSR); // mmnémoniques sys/stat.h: S_IRUSR|S_IWUSR = 0600
    
    strcpy(pseudo, argv[1]);
    strcpy(tubeC2S, strcat(pseudo,"_C2S"));
    mkfifo(tubeC2S,S_IRUSR|S_IWUSR); // mmnémoniques sys/stat.h: S_IRUSR|S_IWUSR = 0600
    
    printf(&pseudo);

    /* (**** à faire ****) demande de connexion au serveur*/
    if(write(ecoute, pseudo, TAILLE_NOM) == -1){
        perror("erreur d'écriture dans écoute");
    }
    ...
    /* (**** à faire ****) initialisations */
        ...
        int clientS2c = open(tubeS2C,O_RDONLY);
        int clientC2s = open(tubeC2S,O_WRONLY);
            ...
            switch(retour) {
                ...
                default :
                    if (FD_ISSET(0, &readfds)) {
                        if ((necrits = read(0,&saisie,TAILLE_SAISIE))>0){
                            write(tubeC2S, &saisie, TAILLE_SAISIE);
                        }  
                    }
                    if (FD_ISSET(clientS2c, &readfds)) {
                        bzero(buf, TAILLE_RECEPTION); // nettoyage
                        if ((nlus = read(tubeS2C,buf,TAILLE_RECEPTION))>0) {
                            int compteur = 0;
                            while(strcmp(buf[TAILLE_MSG*compteur],NULL) !=0){
                                strncpy(message, buf[TAILLE_MSG+compteur], TAILLE_MSG);
