...
public class PointDeVente {
	...
	public int getPrix(Carburant carburant, LocalDateTime date) {
		if (this.prix.containsKey(carburant)) {
			if (this.prix.get(carburant).containsKey(date)) {
				return prix.get(carburant).get(date) ;
			}
			else {
				if (this.prix.get(carburant).lowerKey(date) != null) {
					return this.prix.get(carburant).lowerEntry(date).getValue();
				}
			}
		}
		return 0;
