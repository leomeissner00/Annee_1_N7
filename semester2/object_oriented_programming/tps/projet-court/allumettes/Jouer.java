...
public class Jouer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String aSeparer = args[0] + "@" + args[1];
			String[] separer = aSeparer.split("@");
			if (separer.length != Jeu.PRISE_MAX + 1) {
				throw new ConfigurationException("Le format des joueurs"
						+ " n'est pas le bon.");
			}
			...
			// Construire l'aritre
			Arbitre arbitreJeu = new Arbitre(j1, j2);
			arbitreJeu.setConfiant(confiant);

			// Construire le jeu
			JeuAllumettes jeu = new JeuAllumettes(13);

			arbitreJeu.arbitrer(jeu);
