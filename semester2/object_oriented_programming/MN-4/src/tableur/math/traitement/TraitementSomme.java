...
public class TraitementSomme implements Traitement{

	private Tableur tab;
	
	private double membre;

	public TraitementSomme(Tableur tableur, double nb) {
		this.tab = tableur;
		this.membre = nb;
	}
	...
	public void traiter(Position pos1, Position pos2) throws NoFormuleException {
		...
		for (int i = pos1.x; i <= x_arrivee; i++) {
			for (int j = pos1.y; j <= y_arrivee; j++) {
				...
				((CelluleFormule) cellactif).setFormule(new FormuleMultiplication(form1, this.membre));		
			}
		}
	}
