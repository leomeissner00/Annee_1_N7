...
for i = 0:10
    ...
    TEB_thi = (1/2)*(qfunc(sqrt((2/5)*Eb_div_N0))+qfunc(sqrt((18/5)*Eb_div_N0)));
    TEB_th = [TEB_th TEB_thi];
...
peigne = kron(dirac, [1 zeros(1, Ns-1)]);

xe_ega = filter(h, 1, peigne);
xc_ega = filter(hc, 1, xe_ega);
r_ega = filter(xc_ega, 1, xc_ega);
r_ech_ega = r_ega(n0:Ns:end);

%Toeplitz
Z = toeplitz(r_ech_ega, [r_ech_ega(1) zeros(1,length(r_ech_ega)-1)]);
